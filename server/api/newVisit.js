...
export default defineEventHandler(async (event) => {
  ...
  const doctorsCollection = database.collection('Visit');
  
  // Parse the JSON body
  const newVisitData = await readBody(event);

  try {
    // Insert the new visit data into the collection
    const result = await doctorsCollection.insertOne(newVisitData);
    ...
    return {
      statusCode: 201,
      body: { message: 'Visit created successfully', visitId: result.insertedId },
      headers: {
